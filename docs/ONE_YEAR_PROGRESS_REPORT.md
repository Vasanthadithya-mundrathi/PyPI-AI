...
| 2025-08-01 to 2025-08-15 | Prepared problem statement, objectives, and safety model. | Decided that untrusted package code must never be executed. | Some review ideas were unsafe because they resembled real credential theft. | Replaced them with safe synthetic suspicious fixtures. |
...
| 2025-10-01 to 2025-10-15 | Implemented project skeleton and first CLI design plan. | CLI-first approach selected for review-friendly explanation. | Dashboard-first idea delayed core scanner work. | Deferred dashboard and extension until CLI is complete. |
| 2025-10-16 to 2025-10-31 | Added welcome/about screen design with project identity, developers, domain, safety, and quick commands. | Made tool easier to explain during viva and review. | Initial output was plain and not visually strong. | Added ASCII-art identity and Rich panels/tables. |
...
| 2025-12-16 to 2025-12-31 | Review 1 stabilization and documentation. | Core scanner review run became possible with safe fixtures. | Coverage dropped below target during CLI expansion. | Added tests for CLI commands and archive scanning. |
...
| 2026-02-01 to 2026-02-15 | Added debug and review-mode CLI requirements. | Review output can show scan plan, rule trace, evidence table, and risk breakdown. | Reviewers may not see why a risk score was assigned. | Added `--explain-risk`, `--show-evidence`, and `--trace-rules`. |
...
| 2026-04-01 to 2026-04-15 | Added safe review fixtures: benign, environment/network, setup subprocess, obfuscated. | Review workflow covers low risk, credential-access risk, install-time risk, and obfuscation risk. | Real malicious packages could not be used safely. | Kept real package names as references only, not samples. |
...
| Review | Branch | Main Deliverable | Review Result |
|---|---|---|---|
| Review 1 | `review-one` | CLI, about screen, safe extraction, static scanner, JSON evidence report. | Scanner can detect suspicious APIs in safe fixtures. |
...
| Final Review | `final-review` | Complete tested project with setup scripts, CI, docs, safe examples, final reports, and generated submission assets. | End-to-end CLI review run is reproducible from a clean setup. |
...
- Safe synthetic review fixtures.
